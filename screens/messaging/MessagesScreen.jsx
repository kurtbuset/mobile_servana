...
export default function MessagesScreen() {
  ...
  const { token, isLoading: tokenLoading } = useSecureToken();
  ...
  } = useChatGroup(token, clientId);

  // Departments (for initial selection)
  const { departments, loadDepartments } = useDepartments(token);
  ...
  const {
    ...
    shouldAutoScroll,
    setShouldAutoScroll,
  } = useMessageHistory(chatGroupId, token, flatListRef);
  ...
  useEffect(() => {
    if (tokenLoading || !token || !clientId) return;
